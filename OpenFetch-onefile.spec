...
"""Fail-closed PyInstaller ONE-FILE recipe for the self-updating OpenFetch release.

This spec builds the executable published by the owner-authorized one-file
release workflow and installed by the automatic updater, which cannot install
the one-folder layout. It mirrors `OpenFetch.spec` exactly — the same pinned
build inputs, provider/PyQt exclusions, and audited Qt/PySide filtering — and
only changes the packaging form to the traditional one-file executable.

`OpenFetch.spec` (one-folder) remains the public-compliance candidate recipe and
must not be replaced by this file. General compliance status remains HOLD; this
spec does not alter any compliance document or gate.
"""
